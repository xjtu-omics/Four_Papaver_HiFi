...
def main():
    ...
    for i in name_list:
        print(i)
        info = i.split('_')
        lastz_dir = workdir + '/' + info[0] + '_' + info[1] + '_' + info[2] + '_' + info[3]
        name_info = info
        name_lastz_set = set()

        for j in range(chr_number):
            lastz_chr_file = lastz_dir + '/chr' + str(j+1) + '.xls'
        ...
        for j in name_lastz_set:
            ...
            if (i+'$'+j in edge_set) or (j+'$'+i in edge_set):
                ...
                edge_set.add(i+'$'+j)
        count += 1
    edge_file = workdir+'/con_edge.80.80.'+file_index+'.txt'
